<?php
	...
	//sve kupovine sa korisnicima i knjigama
...
Flight::route('GET /korisnik.json', function() {
	...
	Flight::route('PUT /knjiga/@knjigaID', function($knjigaID) {
		...
		if($podaci == null) {
			...
			if(!property_exists($podaci,'knjigaNaziv') || !property_exists($podaci,'knjigaCena') || !property_exists($podaci,'knjigaTiraz') ||  !property_exists($podaci,'knjigaStanje') || !property_exists($podaci,'pisacID')) {
				...
				if($db->update("knjiga", $knjigaID, array('knjigaNaziv','knjigaIzdanje','knjigaCena','knjigaStanje','pisacID'),array($podaci->knjigaNaziv, $podaci->knjigaIzdanje,$podaci->knjigaCena, $podaci->knjigaStanje,$podaci->pisacID))) {
	...
	Flight::route('POST /knjiga', function() {
		...
		return false;
			...
			if(!property_exists($podaci,'knjigaNaziv') || !property_exists($podaci,'knjigaIzdanje') || !property_exists($podaci,'knjigaCena') ||  !property_exists($podaci,'knjigaStanje') || !property_exists($podaci,'pisacID')) {
				...
				if($db->insert("knjiga","knjigaNaziv,knjigaIzdanje,knjigaCena,knjigaStanje,pisacID",array($podaci_query['knjigaNaziv'], $podaci_query['knjigaIzdanje'],$podaci_query['knjigaCena'],$podaci_query['knjigaStanje'],$podaci_query['pisacID']))) {
